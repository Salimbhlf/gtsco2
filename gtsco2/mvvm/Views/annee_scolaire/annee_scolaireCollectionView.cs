...
namespace gtsco2.mvvm.Views.annee_scolaireCollectionView{
    public partial class annee_scolaireCollectionView : XtraForm {
        public annee_scolaireCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<gtsco2.mvvm.ViewModels.annee_scolaireCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as gtsco2.basededonne.annee_scolaire,
