...
namespace gtsco2.mvvm.Views.BranchCollectionView{
    public partial class BranchCollectionView : XtraForm
    {
        public BranchCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<gtsco2.mvvm.ViewModels.BranchCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as gtsco2.basededonne.Branch,
