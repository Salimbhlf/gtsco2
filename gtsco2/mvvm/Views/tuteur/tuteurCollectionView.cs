...
namespace gtsco2.mvvm.Views.tuteurCollectionView{
    public partial class tuteurCollectionView : XtraForm
    {
        public tuteurCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<gtsco2.mvvm.ViewModels.tuteurCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as gtsco2.basededonne.tuteur,
